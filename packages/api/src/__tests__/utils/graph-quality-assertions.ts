...
export function assertMinimumGraphSize(
  ...
  expect(
    nodes.length,
    `Expected at least ${minNodes} nodes, got ${nodes.length}`
  ).toBeGreaterThanOrEqual(minNodes)
  expect(
    edges.length,
    `Expected at least ${minEdges} edges, got ${edges.length}`
 ...
 */
export function assertGraphMetadata(
  ...
  expect(
    metadata.totalNodes,
    `Metadata totalNodes (${metadata.totalNodes}) !== actual nodes (${expectedNodes})`
  ).toBe(expectedNodes)
  expect(
    metadata.totalEdges,
    `Metadata totalEdges (${metadata.totalEdges}) !== actual edges (${expectedEdges})`
  ).toBe(expectedEdges)
}
...
export function assertLODConsistency(nodes: Neo4jNode[]) {
  ...
  classNodes.forEach((node) => {
    expect(
      node.lod,
      `Class node '${node.id}' should have LOD <= 1, got ${node.lod}`
  ...
  functionNodes.forEach((node) => {
    expect(
      node.lod,
      `Function node '${node.id}' should have LOD <= 2, got ${node.lod}`
  ...
  methodNodes.forEach((node) => {
    expect(
      node.lod,
      `Method node '${node.id}' should have LOD <= 3, got ${node.lod}`
  ...
  variableNodes.forEach((node) => {
    expect(
      node.lod,
      `Variable node '${node.id}' should have LOD <= 4, got ${node.lod}`
 ...
 */
export function assertGraphQuality(
  graph: {
    nodes: Neo4jNode[]
    edges: Neo4jEdge[]
    metadata?: GraphMetadata
