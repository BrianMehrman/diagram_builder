 ...
 */
async function triggerParserImport(codebaseId: string, input: CreateCodebaseInput): Promise<void> {
 ...
 */
export async function getCodebaseById(workspaceId: string, codebaseId: string): Promise<Codebase> {
  // Check cache first
  ...
  if (row.workspaceId !== workspaceId) {
    throw new NotFoundError(
      'Codebase not found',
      `Codebase ${codebaseId} not found in workspace ${workspaceId}`
    )
  }
 ...
 */
export async function deleteCodebase(workspaceId: string, codebaseId: string): Promise<void> {
  ...
  const workspaceCodebasesKey = buildCacheKey('workspace', `${workspaceId}:codebases`)
  await Promise.all([cache.invalidate(codebaseCacheKey), cache.invalidate(workspaceCodebasesKey)])
}
