...
describe('Workspace Endpoints', () => {
  ...
  describe('Authentication', () => {
    it('should return 401 for unauthenticated POST /api/workspaces', async () => {
      const response = await request(app).post('/api/workspaces').send({ name: 'Test Workspace' })
    ...
    it('should return 401 for unauthenticated PUT /api/workspaces/:id', async () => {
      const response = await request(app).put('/api/workspaces/test-id').send({ name: 'Updated' })
      expect(response.status).toBe(401)
    })
  ...
  describe('POST /api/workspaces/:workspaceId/codebases', () => {
    ...
    it('should require authentication', async () => {
      const response = await request(app).post(`/api/workspaces/${workspaceId}/codebases`).send({
        source: '/path/to/repo',
        type: 'local',
      })
  ...
  describe('GET /api/workspaces/:workspaceId/codebases', () => {
    ...
    it('should require authentication', async () => {
      const response = await request(app).get(`/api/workspaces/${workspaceId}/codebases`)
