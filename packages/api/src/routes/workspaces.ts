...
workspacesRouter.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const userId = req.user?.userId
    logger.info('loading for ', { userId })
    if (!userId) {
      throw new ValidationError('Invalid request', 'User ID not found in token')
    }
...
workspacesRouter.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!workspace) {
      throw new NotFoundError('Workspace not found', `Workspace with ID ${id} does not exist`)
    }
 ...
 */
workspacesRouter.get(
  '/:workspaceId/codebases/:codebaseId',
  authenticate,
  asyncHandler(async (req: Request, res: Response) => {
