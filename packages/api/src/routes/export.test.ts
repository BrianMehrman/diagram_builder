...
describe('Export Endpoints', () => {
  ...
  describe('Authentication', () => {
    it('should return 401 for unauthenticated POST /api/export/plantuml', async () => {
      const response = await request(app).post('/api/export/plantuml').send({ repoId: 'test-id' })
    ...
    it('should return 401 for unauthenticated POST /api/export/mermaid', async () => {
      const response = await request(app).post('/api/export/mermaid').send({ repoId: 'test-id' })

      expect(response.status).toBe(401)
    })

    it('should return 401 for unauthenticated POST /api/export/drawio', async () => {
      const response = await request(app).post('/api/export/drawio').send({ repoId: 'test-id' })

      expect(response.status).toBe(401)
    })

    it('should return 401 for unauthenticated POST /api/export/gltf', async () => {
      const response = await request(app).post('/api/export/gltf').send({ repoId: 'test-id' })
