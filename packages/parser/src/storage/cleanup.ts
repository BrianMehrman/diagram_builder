...
export async function runCleanupJob(options: CleanupOptions = {}): Promise<number> {
  ...
  for (const { workspaceId, codebaseId, metadata } of codebases) {
    ...
    if (ageMs > maxAgeMs) {
      ...
      if (dryRun) {
        ...
        logger.info(`Cleaning up codebase ${codebaseId} (age: ${ageDays} days, size: ${sizeMB} MB)`)
  ...
  logger.info(
    `Cleanup job complete. ${dryRun ? 'Would cleanup' : 'Cleaned up'} ${cleanedCount} codebases`
  )
