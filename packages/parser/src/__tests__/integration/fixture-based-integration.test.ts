...
function validateGraphQuality(graph: IVMGraph, expectedMinNodes: number, expectedMinEdges: number) {
  ...
  graph.nodes.forEach((node: IVMNode) => {
    ...
    expect(['file', 'class', 'function', 'variable', 'interface', 'type', 'method']).toContain(
      node.type
    )
  ...
  if (expectedMinEdges > 0) {
    ...
    graph.edges.forEach((edge: IVMEdge) => {
      ...
      expect(['imports', 'exports', 'depends_on', 'contains', 'calls', 'inherits']).toContain(
        edge.type
      )
...
describe('Parser Integration Tests - Real Fixtures', () => {
  ...
  describe('Error Handling', () => {
    it('should reject invalid repository path', async () => {
      ...
      await expect(loadRepository(invalidPath, { extensions: ['.ts'] })).rejects.toThrow()
    })
    ...
    it('should reject non-directory paths', async () => {
      ...
      await expect(loadRepository(filePath, { extensions: ['.ts'] })).rejects.toThrow()
    })
  })
