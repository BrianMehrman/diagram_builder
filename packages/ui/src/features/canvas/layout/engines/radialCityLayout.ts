...
export class RadialCityLayoutEngine implements LayoutEngine {
  ...
  layout(graph: IVMGraph, config: RadialCityLayoutConfig = {}): HierarchicalLayoutResult {
    ...
    const ringAssignments = distributeDistrictsAcrossRings(districtNodes, nodeDepths, {
      centerRadius: effectiveCenterRadius,
    ...
    const { fileBlocks, orphans } = buildFileBlockHierarchy(allInternalForHierarchy)
    ...
    for (const [depth, assignments] of assignmentsByDepth) {
      ...
      for (let i = 0; i < arcs.length; i++) {
        ...
        if (isCompound && assignment.nodeIds.length <= 3) {
          ...
          const positioned = positionBlocksInArc(blockDescriptors, arc.arcStart, arc.arcEnd, radius)
        ...
        if (districtOrphanNodes.length > 0 || districtOrphans.length > 0) {
          ...
          if (allOrphansForDistrict.length > 0) {
            ...
            const orphanChildren = placeChildrenInGrid(allOrphansForDistrict, orphanFootprint)
    ...
    if (externalNodes.length > 0) {
      const maxDepth = Math.max(0, ...fileNodes.map((n) => effectiveDepths.get(n.id) ?? 0))
      const externalRingDepth = maxDepth + 1
      const baseExternalRadius =
        calculateRingRadius(externalRingDepth, {
          centerRadius: effectiveCenterRadius,
          ringSpacing: effectiveRingSpacing,
        }) + radiusOffset
      ...
      for (const [zoneType, nodes] of [...zoneGroups.entries()].sort(([a], [b]) =>
        a.localeCompare(b)
      )) {
