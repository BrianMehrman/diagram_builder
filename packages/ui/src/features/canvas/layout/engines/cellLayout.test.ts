...
describe('CellLayoutEngine', () => {
  ...
  describe('layout', () => {
    ...
    it('should position variable nodes closer to center than methods', () => {
      ...
      const varDistances = ['v1', 'v2'].map((id) => distance(center, result.positions.get(id)!))
    ...
    it('should return empty positions when no cell node found', () => {
      const graph = makeGraph([makeNode('m1', 'method', 'orphan')])
    ...
    it('should compute spherical bounds around the cell', () => {
      ...
      expect(result.bounds.min).toEqual({
        x: -membraneRadius,
        y: -membraneRadius,
        z: -membraneRadius,
      })
