...
function makeBlock(fileId: string, x: number, z: number, width = 4): BlockLayout {
  return {
    fileId,
    position: { x, y: 0, z },
...
describe('refineDistrictProximity', () => {
  ...
  it('pushes overlapping blocks apart', () => {
    const blocks = [makeBlock('f1', 0, 0, 6), makeBlock('f2', 1, 0, 6)]
  ...
  it('produces deterministic output with same seed', () => {
    const blocks = [makeBlock('f1', 5, 5, 6), makeBlock('f2', 8, 5, 6), makeBlock('f3', 5, 8, 6)]
    const edges = [makeEdge('f1', 'f2')]
  ...
  it('produces different output with different seeds', () => {
    const blocks = [makeBlock('f1', 5, 5, 6), makeBlock('f2', 8, 5, 6)]
    ...
    const anyDiff = result1.some(
      (b, i) => b.position.x !== result2[i]!.position.x || b.position.z !== result2[i]!.position.z
    )
  ...
  it('preserves y=0 for all blocks', () => {
    const blocks = [makeBlock('f1', 0, 0), makeBlock('f2', 5, 5)]
