...
describe('RadialCityLayoutEngine', () => {
  ...
  describe('layout', () => {
    ...
    it('should apply custom config values', () => {
      ...
      const smallDist = Math.sqrt(
        smallResult.positions.get('f2')!.x ** 2 + smallResult.positions.get('f2')!.z ** 2
      )
      const largeDist = Math.sqrt(
        largeResult.positions.get('f2')!.x ** 2 + largeResult.positions.get('f2')!.z ** 2
      )
  ...
  describe('hierarchical output', () => {
    ...
    it('should place child nodes inside file blocks', () => {
      ...
      const district = result.districts.find((d) => d.blocks.some((b) => b.fileId === 'f2'))
  ...
  describe('performance', () => {
    it('should layout 1000 nodes in under 50ms', () => {
      ...
      // Add entry point
      graph.nodes.unshift(makeFileNode('entry', 'src/index.ts', { depth: 0, path: 'src/index.ts' }))
  ...
  describe('determinism (hierarchical)', () => {
    it('should produce identical hierarchical output across 10 runs', () => {
      const graph: Graph = {
        ...
        edges: [{ id: 'e1', source: 'f2', target: 'f3', type: 'imports', metadata: {} }],
      ...
      for (let i = 0; i < 9; i++) {
        ...
        for (let d = 0; d < baseline.districts.length; d++) {
          expect(result.districts[d]!.blocks.length).toBe(baseline.districts[d]!.blocks.length)
        }
      }
    })
  })
