  ...
  return (
    <group position={[position.x, position.y, position.z]}>
      ...
      <mesh position={[0, buildingHeight / 2, 0]}>
        ...
        <meshStandardMaterial color="#475569" transparent opacity={xrayOpacity} wireframe />
      </mesh>
      ...
      {showDetail && (
        <group>
          ...
          {classes.map((cls, i) => (
            ...
            >
              <planeGeometry args={[BUILDING_WIDTH * 0.9, BUILDING_DEPTH * 0.9]} />
              <meshStandardMaterial color="#3b82f6" transparent opacity={0.3} side={DoubleSide} />
            </mesh>
          ))}
          ...
          {methods.map((method, i) => {
            ...
            return (
              <mesh
                key={method.id}
                position={[Math.cos(angle) * radius, FLOOR_HEIGHT * 1.5, Math.sin(angle) * radius]}
              >
