...
describe('cityViewUtils', () => {
  ...
  describe('getDirectoryFromLabel', () => {
    it('should extract directory from path with slashes', () => {
      expect(getDirectoryFromLabel('src/components/Button.tsx')).toBe('src/components')
    })
    ...
    it('should handle deeply nested paths', () => {
      expect(getDirectoryFromLabel('src/features/canvas/views/CityView.tsx')).toBe(
  ...
  describe('isBaseClass', () => {
    ...
    it('works with cross-namespace node IDs (AC-5)', () => {
      const edges = [{ source: 'pkg-a::ChildClass', target: 'pkg-b::BaseClass', type: 'extends' }]
  ...
  describe('buildIncomingEdgeCounts', () => {
    ...
    it('accumulates multiple edges to the same target', () => {
      const counts = buildIncomingEdgeCounts([{ target: 'A' }, { target: 'A' }, { target: 'A' }])
