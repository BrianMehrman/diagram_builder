...
import { computeTunnelRadius, generateTunnelPoints } from '../undergroundUtils'
...
export function DependencyTunnel({
  ...
  const curve = useMemo(() => {
    ...
    return new CatmullRomCurve3(points.map((p) => new Vector3(p.x, p.y, p.z)))
  ...
  return (
    <group>
      ...
      <mesh>
        ...
        <meshBasicMaterial color={color} transparent opacity={0.1} side={BackSide} />
      </mesh>
    </group>
  )
}
