 ...
 */
export function getLogScaledHeight(
...
export function applyFloorBandColors(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const floorIndex = Math.min(Math.floor(normalized * floorCount), floorCount - 1)
...
export function getMethodCount(node: IVMNode): number {
  ...
  const metaMethods = (node.metadata.properties as Record<string, unknown> | undefined)?.methods
