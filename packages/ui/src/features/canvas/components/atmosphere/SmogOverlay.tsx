 ...
 */
function generateParticlePositions(arc: DistrictArcMetadata, count: number): THREE.Vector3[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    positions.push(
      new THREE.Vector3(Math.cos(angle) * r, SMOG_HEIGHT + yOffset, Math.sin(angle) * r)
    )
  }
...
export function SmogOverlay({ districts, districtNodeMap }: SmogOverlayProps) {
  ...
  const smogEnabled = useCanvasStore((s) => s.citySettings.atmosphereOverlays.smog)
      ...
      {smogDistricts.map((sd) => (
        <group key={sd.district.id}>
          {sd.particles.map((pos, i) => (
            <sprite key={i} position={[pos.x, pos.y, pos.z]} scale={[2.5, 1.5, 1]}>
              <spriteMaterial
                color={SMOG_COLOR}
                transparent
                opacity={sd.opacity}
