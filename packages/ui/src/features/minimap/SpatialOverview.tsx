...
function MiniNode({
  ...
  return (
    <group position={[position.x, position.y, position.z]}>
      ...
      >
        <div
          className={`text-white text-[10px] px-1 py-0.5 rounded whitespace-nowrap ${
 ...
 */
function CameraIndicator({ position }: { position: { x: number; y: number; z: number } }) {
  return (
...
function FovIndicator({
  ...
  return <Line points={points} color="#ffffff" lineWidth={1.5} opacity={0.4} transparent />
}
...
export function SpatialOverview({
  ...
  return (
    <div className="w-full h-full bg-gray-900 cursor-crosshair">
      <Canvas>
        <OrthographicCamera makeDefault position={[15, 15, 15]} zoom={8} near={0.1} far={1000} />

        {/* Axes helper lines */}
        <gridHelper args={[20, 20, '#4b5563', '#374151']} />

        {/* Render mini nodes */}
        <MiniNodes nodes={nodes} selectedNodeId={selectedNodeId} onNodeClick={onNodeClick} />
