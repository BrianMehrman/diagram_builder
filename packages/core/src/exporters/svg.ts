...
function generateSVG(
  ...
  const { width, height, padding, showLabels, showEdgeLabels, arrowSize, curvedEdges, autoFit } =
    options
  ...
  for (const edge of edges) {
    ...
    lines.push(
      `    <path d="${path}" fill="none" stroke="${color}" stroke-width="${styling.lineWidth ?? 1}" ${strokeDasharray} ${markerEnd}/>`
    )
    ...
    if (showEdgeLabels && edge.metadata.label) {
      ...
      lines.push(
        `    <text x="${midX}" y="${midY}" text-anchor="middle" font-size="${fontSize - 2}" font-family="${fontFamily}" fill="${colors.text ?? '#333333'}">${escapeXml(edge.metadata.label)}</text>`
      )
    }
  }
  lines.push('  </g>')
  ...
  for (const node of nodes) {
    ...
    lines.push(
      `    <g class="node" data-id="${escapeXml(node.id)}" data-type="${node.type}" ${filter}>`
    )
    lines.push(
      `      ${generateNodeShape(shape, pos.x, pos.y, pos.width, pos.height, fill, stroke, rounded)}`
    )
    ...
    if (showLabels) {
      ...
      const label =
        node.metadata.label.length > 15
          ? node.metadata.label.substring(0, 12) + '...'
          : node.metadata.label
      lines.push(
        `      <text x="${textX}" y="${textY}" text-anchor="middle" font-size="${fontSize}" font-family="${fontFamily}" fill="${colors.text ?? '#333333'}">${escapeXml(label)}</text>`
      )
    }
  ...
  if (options.includeLegend) {
    ...
    lines.push(
      `    <text x="0" y="15" font-size="${fontSize}" font-weight="bold" font-family="${fontFamily}">Legend</text>`
    )
    ...
    for (const type of nodeTypes) {
      ...
      lines.push(
        `    <rect x="0" y="${legendY}" width="12" height="12" fill="${color}" stroke="${colors.border ?? '#CCCCCC'}"/>`
      )
      lines.push(
        `    <text x="18" y="${legendY + 10}" font-size="${fontSize - 2}" font-family="${fontFamily}">${type}</text>`
      )
      legendY += 18
    }
