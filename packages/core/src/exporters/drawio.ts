 ...
 */
const EDGE_TYPE_TO_STYLE: Record<
  EdgeType,
  { startArrow: string; endArrow: string; dashed: boolean }
> = {
 ...
 */
function buildNodeStyle(node: IVMNode, colors: ColorScheme, styling: ExportStyling): string {
  const shape = NODE_TYPE_TO_SHAPE[node.type]
  const baseStyle = SHAPE_TO_STYLE[shape]
  const fillColor = node.style?.color
    ? formatColor(node.style.color)
    : getNodeColor(node.type, colors)
 ...
 */
function buildEdgeStyle(edge: IVMEdge, colors: ColorScheme, styling: ExportStyling): string {
  const { startArrow, endArrow, dashed } = EDGE_TYPE_TO_STYLE[edge.type]
  const strokeColor = edge.style?.color
    ? formatColor(edge.style.color)
    : getEdgeColor(edge.type, colors)
