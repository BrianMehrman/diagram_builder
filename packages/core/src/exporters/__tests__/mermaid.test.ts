...
describe('MermaidExporter', () => {
  ...
  describe('export() - Flowchart', () => {
    ...
    it('should support different directions', () => {
      const graph = createTestGraph([{ id: 'node1', type: 'file', metadata: { label: 'Node 1' } }])
    ...
    it('should add styling for nodes', () => {
      const graph = createTestGraph([{ id: 'file1', type: 'file', metadata: { label: 'File 1' } }])
      const result = exporter.export(graph)
  ...
  describe('export() - Class Diagram', () => {
    ...
    it('should mark enums with <<enumeration>> stereotype', () => {
      const graph = createTestGraph([{ id: 'enum1', type: 'enum', metadata: { label: 'MyEnum' } }])
      const result = exporter.export(graph, { diagramType: 'classDiagram' })
      ...
      const graph = createTestGraph([
        {
          id: 'system1',
          type: 'repository',
          metadata: { label: 'My System', description: 'Main system' },
        },
      ])
      ...
      const graph = createTestGraph([
        {
          id: 'container1',
          type: 'package',
          metadata: { label: 'API Container', description: 'REST API' },
        },
      ])
      ...
      const graph = createTestGraph([
        {
          id: 'comp1',
          type: 'class',
          metadata: { label: 'UserService', description: 'Handles user operations' },
        },
      ])
  ...
  describe('validateOptions()', () => {
    ...
    it('should return error for invalid diagram type', () => {
      const errors = exporter.validateOptions({
        diagramType: 'invalid' as unknown as MermaidDiagramType,
      })
