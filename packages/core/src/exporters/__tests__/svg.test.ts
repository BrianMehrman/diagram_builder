...
function createTestGraph(nodes: IVMNode[] = [], edges: IVMEdge[] = []): IVMGraph {
  return {
    nodes,
    edges,
    metadata: {
...
describe('SVGExporter', () => {
  ...
  describe('validateOptions', () => {
    ...
    it('should validate width minimum', () => {
      expect(exporter.validateOptions({ width: 50 })).toContain('width must be at least 100')
    })

    it('should validate height minimum', () => {
      expect(exporter.validateOptions({ height: 50 })).toContain('height must be at least 100')
    })

    it('should validate nodeWidth minimum', () => {
      expect(exporter.validateOptions({ nodeWidth: 10 })).toContain('nodeWidth must be at least 20')
    })
    ...
    it('should validate scale is positive', () => {
      expect(exporter.validateOptions({ scale: 0 })).toContain('scale must be greater than 0')
      expect(exporter.validateOptions({ scale: -1 })).toContain('scale must be greater than 0')
    })

    it('should validate arrowSize minimum', () => {
      expect(exporter.validateOptions({ arrowSize: 0 })).toContain('arrowSize must be at least 1')
    })
